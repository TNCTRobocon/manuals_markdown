...
一見するとCの構造体のように見えるかもしれない  
まぁほぼほぼ同じものなのだが

構造体と同じくクラスの最後は`;`で終わること

ちなみにC++では`struct`を使って`class`とまったく同じ機能を持つものを作ることができる  
`class`を使いたくない場合や後述する違いを使いたいときに使える
...
`class`を使った場合、アクセス修飾子をつけない場合は自動的に`private`を指定したとみなされる

`struct`の場合アクセス修飾子を付けないと自動的に`public`を指定したことになる
...
カプセル化が破壊されること、setterやgetterのような名前だと値を変える意図が伝わりにくくなるので別のメソッド名がないか検討することを勧める
