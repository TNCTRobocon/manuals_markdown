...
- 定電流ダイオード（CRD）
  ...
  - LEDなどの定電流源にどうぞ
- TVSダイオード
  - 回路を過電圧から保護する素子。ある一定の電圧(数十V)を超えると急に電流が流れるようになる
...
- 余ったエネルギーを熱として消費するため効率が悪く熱くなる(特にリポバッテリーから5V電源を作ろうとするのは厳しい)
...
- ロジックIC
  ...
  - 74シリーズ
    - おそらくもっとも一般的なロジックIC。なぜか部室に大量にある。~~在庫管理頑張って~~
    - 割と歴史のあるシリーズで、古い刻印のものを蒐集したりこれらのみでCPUやらグラフィックボードやらを組む~~変人~~ファンも多い
    - 74HC00
      - 4回路入りNAND
      - 超有名なのでこれくらいは覚えておこう
    - 基板に実装するときはICソケットを使用しよう。
...
- スイッチ類
  ...
  - 非常停止スイッチ
    ...
    - オルタネート型
    - これを電源スイッチとする運用もあるが、NHK側はやめてほしいと表明している。
  - マイクロスイッチ(リミットスイッチ)
    - 突き出している接点が押し込まれるとONするスイッチ。
    - 機構の保護や位置検知に使用。
...
- CAN
  ...
  - 通信速度も速くロボコン向きであることから~~過去幾度も導入しようとした痕跡があるが、全てロステクとなっている~~2023年度現在、UARTとならんで広く使われている。
...
- 割り込み処理
  - 今実行している処理を中断して別の処理を実行する。
  - 時間経過、特定のピンの入力の状態などで発生させることができる。
...
- STM32
  - 強い奴
  - nucleoボードに乗ってるやつら。最近は自作の基板に乗っていることも多い。
  - 名前の通り32bitのCPUでクロックも高いものが多い。つまり強い。
  - 少々お高い…が製造元のSTmicroが行うユニバーシティプログラムでタダで貰える。
  - ロボコン界隈ではよく見る
  - ロボAでは場面に合わせて3種類を使い分けている。
  - ロボAではcube IDEで開発を行っている。高性能なぶん、扱いは難しい。
- arduino
  ...
  - arduino nano
    - 小さくなったarduino UNO
    - あんまりスペースを確保できない時に使う
    - 手軽に5V電源を取り出せるのでブレッドボードの電源に使うこともできる
  - arduino mega　
    - デカくなったarduino
    - ピン数が非常に多くロボコンでもよく使う(使われていた)
- teensy
...
- Raspberry Pi
  - 要するにちっちゃくてお安いPC。  
    - ちゃんとOSも入るし、インターネットにも繋がる。
    - ロボットをWi-Fiでコントロールするときによく使われる。
  - GPIOやUART等の各種通信もできるので割と万能。ただしarduinoと比べると扱いはちょっと難しい。
- Raspberry Pi Pico
  - 名前で勘違いしやすいが、ラズパイとは性能もできることも異なる
  - マルチコア。つまり、つよい。
  - platform IOやarduino IDEを使用することでarduino言語で開発できる
  - arduino nanoとSTM32に立場を奪われているかわいそうな子。

- ESP32
  - 最近検索数がarduinoを超えたとか何とかで結構アツい~~らしい~~マイコン
  - CPUの性能も高く、WiFiやbluetoothを内蔵しているため様々な用途に使える。
  - チップ単体だとけっこうお安い。arduinoと同じように開発できるのもうれしい。
  - デュアルコアの製品もある
  - 無線を使いたいときにどうぞ
- TWELITE
  - 簡単に無線を使えるマイコン
...
- マブチモーター
  - ロボコンでは一般的なDCモーター。要するに超つよい模型用モーター。
  - 毎年タダでくれる。
  - 定格以上でぶん回すものby設計者M

- ロボマスモーター
  - 最近はやりのブラシレスDCモーター
  - 専用のモータードライバが必要。あと少々お高い(マブチモーター比)
  - 小型(軽い)のわりにつよいのが売り

- 電磁弁
  - エアを流すor流さないでエアシリンダを制御する部品
  - 中身にコイルが入っているのでフライホイールダイオードを付けよう。
...
- 旧部室
  - 先輩方の話にたまに出てくる、いまの部室がリニューアルされる前の姿。
  - まあ狭くて汚かった。teamsを漁ると当時の写真が出てくる。
- 回路電源
  - 回路の机の上にあるテーブルタップ。
  - スイッチを消し忘れる(特にはんだごて)と真面目に火事になりかねないので注意。
...
- 多読
  ...
  - 修行だと思って頑張ってください♡
  - Google翻訳のPDF翻訳機能とブラウザの単語検索機能で楽ができる。
- 神
  - 設計製作回路制御全部できる人
- ギニョリズム
  - 取敢えず下ネタ言っとけばいいの精神
...
- 水筒コンデンサ
  - 水筒のように大きなコンデンサ
  - 一般的に端子がねじ端子になっている
  - 部室の外に生息する野生の水筒コンデンサの端子を不用意に触っていはいけない。(たまっていた電荷で感電する可能性がある)
    - 部室のものはだいじょうぶ。
  - 学校に持ってきて文鎮にすると教員をドン引きさせられるので是非やってみよう！！！！
- 過去の遺産
  - 部室にある謎の部品や基板たち
  - 何がどれだけあるか把握しきれている人は恐らくいない
